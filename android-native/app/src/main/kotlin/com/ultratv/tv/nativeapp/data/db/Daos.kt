...
/**
 * FTS-backed search. We use a unicode61 tokenizer so "café" matches "cafe",
 * and append `*` to the user's query before MATCH for prefix search ("acti"
 * matches "Action"). Caller still passes a raw query; this DAO sanitises.
 *
 * Population: every upsertAll() on the content DAOs also inserts into the
 * matching _fts table — see CatalogRepository. Rebuild on destructive
 * migration is automatic because Room recreates the schema.
 */
@Dao
interface SearchDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertMovieFts(rows: List<MovieFts>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertSeriesFts(rows: List<SeriesFts>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertChannelFts(rows: List<ChannelFts>)

    @Query("DELETE FROM movie_fts WHERE rowid IN (SELECT id FROM movie WHERE providerId = :pid)")
    suspend fun clearMovieFtsForProvider(pid: Long)

    @Query("DELETE FROM series_fts WHERE rowid IN (SELECT id FROM series WHERE providerId = :pid)")
    suspend fun clearSeriesFtsForProvider(pid: Long)

    @Query("DELETE FROM channel_fts WHERE rowid IN (SELECT id FROM channel WHERE providerId = :pid)")
    suspend fun clearChannelFtsForProvider(pid: Long)

    @Query("""
        SELECT m.* FROM movie m
        JOIN movie_fts f ON m.id = f.rowid
        WHERE m.providerId = :pid AND movie_fts MATCH :ftsQuery
        LIMIT :limit
    """)
    suspend fun searchMovies(pid: Long, ftsQuery: String, limit: Int = 100): List<MovieEntity>

    @Query("""
        SELECT s.* FROM series s
        JOIN series_fts f ON s.id = f.rowid
        WHERE s.providerId = :pid AND series_fts MATCH :ftsQuery
        LIMIT :limit
    """)
    suspend fun searchSeries(pid: Long, ftsQuery: String, limit: Int = 100): List<SeriesEntity>

    @Query("""
        SELECT c.* FROM channel c
        JOIN channel_fts f ON c.id = f.rowid
        WHERE c.providerId = :pid AND channel_fts MATCH :ftsQuery
        LIMIT :limit
    """)
    suspend fun searchChannels(pid: Long, ftsQuery: String, limit: Int = 100): List<ChannelEntity>
}
